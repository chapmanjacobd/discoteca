  ...
  playlistLoaded(index, data, previousDetails) {
    ...
    if (details.live || previousDetails != null && previousDetails.live) {
      ...
      if (details.misses >= this.hls.config.liveMaxUnchangedPlaylistRefresh) {
        ...
        const error = new Error(`levelOrTrack (${levelOrTrack.id}) hits max allowed unchanged reloads.`);
        this.warn(error);
        const {
          networkDetails,
          context
...
class BaseStreamController extends TaskLoop {
  ...
  */
  ...
  calculateOptimalSwitchPoint(nextLevel, bufferInfo) {
    ...
    const currentTime = ((_this$media4 = this.media) == null ? void 0 : _this$media4.currentTime) || this.getLoadPosition();
  ...
  scheduleTrackSwitch(bufferInfo, fetchdelay, okToFlushForwardBuffer) {
    ...
    const bufferedFrag = okToFlushForwardBuffer ? this.getBufferedFrag(this.getLoadPosition() + fetchdelay) : null;
    if (bufferedFrag) {
    ...
    const operation = {
      ...
      onError: error => {
        ...
        if (error.code === DOMException.QUOTA_EXCEEDED_ERR || error.name == 'QuotaExceededError' || `quota` in error) {
          ...
          */
          this.warn(`Failed ${appendErrorCount}/${this.hls.config.appendErrorMaxRetry} times to append segment in "${type}" sourceBuffer with error: ${error.message}`);
          ...
          case pmtId:
            {
              ...
              const parsedPIDs = parsePMT(data, offset, this.typeSupported, isSampleAes, this.observer, this.logger, this.config);
